...
function sp_query_string($tourniquet)
{
	...
	$finds = array(
		'text1' => ', Simple Machines LLC</a>',
		'text2' => ', Simple Machines</a>',
	);
	$replaces = array(
		'text1' => ', Simple Machines LLC</a><br />' . $fix,
		'text2' => ', Simple Machines</a><br />' . $fix,
	);

	// replacements can only occur once from the bottom up
	foreach($finds as $key => $find)
	{
		$pos = strrpos($tourniquet, $find);
		if ($pos !== false && strpos($tourniquet, $fix) === false)
		{
...
function sportal_html_headers($html_headers, $element = 'head')
{
	...
	{
		var spDoc = document.getElementsByTagName("' . strtoupper($element) . '")[0];';
