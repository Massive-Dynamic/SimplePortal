...
function sportal_init($standalone = false)
{
	...
	$sportal_version = !empty($modSettings['sp_version']) ? $modSettings['sp_version'] : '2.3.8';
...
function sp_query_string($tourniquet)
{
	...
	$finds = array(
		'li1' => '<li class="copyright">',
		'hyperlink1' => ', Simple Machines LLC</a>',
		'hyperlink2' => ', <a href="http://www.simplemachines.org" title="Simple Machines" target="_blank" class="new_win">Simple Machines</a>',
		'div1' => '<div id="footer">',
		'div2' => '<div class="frame">',
		'class1' => 'class="copywrite"',
		'class2' => 'class="copyright"',
	);
	$replaces = array(
		'li1' => '<li class="copyright" style="line-height: 1.5em;"><span class="smalltext" style="display: inline; visibility: visible;">' . $fix .'</span></li><li class="copyright" style="line-height: 1.5em;">',
		'hyperlink1' => ', Simple Machines LLC</a>' . $fix,
		'hyperlink2' => ', <a href="http://www.simplemachines.org" title="Simple Machines" target="_blank" class="new_win">Simple Machines</a><br />' . $fix,
		'div1' => '<div id="footer"><div style="text-align: center; width: 100%; font-size: x-small; margin-bottom: 5px;">' . $fix . '</div>',
		'div2' => '<div class="frame"><div style="text-align: center; width: 100%; font-size: x-small; margin-bottom: 5px;">' . $fix . '</div>',
		'class1' => 'class="copywrite" style="line-height: 1em;"',
		'class2' => 'class="copyright" style="line-height: 1.5em;"',
	);

	// some replacements can only occur once from the bottom up
	foreach($finds as $key => $find)
	{
		$pos = strrpos($tourniquet, $find);
		if ($pos !== false && strpos($tourniquet, $fix) === false && strpos($key, 'class') === false)
			$tourniquet = substr_replace($tourniquet, $replaces[$key], $pos, strlen($find));
		elseif (strpos($tourniquet, $fix) !== false && strpos($key, 'class') !== false)
			$tourniquet = str_replace($find, $replaces[$key], $tourniquet);
	}

	// when all else fails, place it as the last child of the page body :-(
	if (stripos($tourniquet, $fix) === false)
	{
		$fix = '<div style="text-align: center; width: 100%; font-size: x-small; margin-bottom: 5px;">' . $fix . '</div>';
		$pos = strrpos($tourniquet, '</body>');
		$tourniquet = substr_replace($tourniquet, $fix . '</body>', $pos, 7);
	}
...
function sportal_parse_page($body, $type)
{
	...
	elseif ($type == 'php')
	{
		...
		$body = trim($body, '<?php');
		$body = trim($body, '?>');
		$tempHeaders = $context['html_headers'];
		$context['html_headers'] = '';
		eval($body);
		print(sportal_html_headers($context['html_headers']));
		$context['html_headers'] = $tempHeaders . $context['html_headers'];
	}
}
...
function sportal_html_headers($html_headers, $element = 'head')
{
	if (empty($html_headers))
		return '';

	list($parentElements, $script_tags['src'], $script_tags['text'], $allTags, $x) = array(array('div'), array(), array(), array(), 0);
	libxml_use_internal_errors(true);
	$dom = new DOMDocument();
	if($dom->loadHTML($html_headers,LIBXML_HTML_NOIMPLIED|LIBXML_HTML_NODEFDTD))
	{
		// parse all tags that aren't script & then remove them
		foreach($dom->getElementsByTagName('*') as $node)
		{
			if ($node->tagName != ($element != 'body' ? 'body' : 'head') && $node->tagName != 'html' && $node->tagName != 'script')
			{
				$allTags[] = $dom->saveHTML($node);
				$node->parentNode->removeChild($node);
				// just allow the parent container once which includes its children
				if (in_array($node->tagName, $parentElements))
					break;
			}
		}

		// parse script tags & then remove them
		$script_elements = $dom->getElementsByTagName('script');
		foreach ($script_elements as $script)
		{
			if($script->getAttribute('src'))
				$script_tags['src'][] = $script->getAttribute('src');
			else
				$script_tags['text'][] = $script->nodeValue;
		}
		foreach(iterator_to_array($dom->getElementsByTagName('script')) as $node)
			$node->parentNode->removeChild($node);
	}
	$html_headers = $dom->saveHTML();

	$output = '
	<script type="text/javascript"><!-- // --><![CDATA[
	function sp_insertPageHeaders_' . $element. '()
	{
		var spDoc = document.getElementsByTagName("' . $element . '")[0];';

	foreach ($script_tags['src'] as $src)
	{
		$x++;
		$output .= '
		var new' . $x . ' = document.createElement("script");
		new' . $x . '.type = "text/javascript";
		new' . $x . '.src = "' . $src . '";
		spDoc.appendChild(new' . $x . ');';
	}
	foreach ($script_tags['text'] as $text)
	{
		$x++;
		$output .= '
		var new' . $x . ' = document.createElement("script");
		new' . $x . '.type = "text/javascript";
		var tnode' . $x . ' = document.createTextNode(("' . preg_replace("/\r?\n/", "\\n", addslashes($text)) . '"));
		new' . $x . '.appendChild(tnode' . $x . ');
		spDoc.appendChild(new' . $x . ');';
	}
	if (!empty($allTags))
	{
		...
		$output .= '
		var spDocLast = document.' . $element . '.childNodes[document.' . $element . '.childNodes.length-1];
	...
	if (window.addEventListener)
		window.addEventListener("load", sp_insertPageHeaders_' . $element. ', false);
	else if (window.attachEvent)
		window.attachEvent("onload", sp_insertPageHeaders_' . $element. ');
	// ]]></script>';
